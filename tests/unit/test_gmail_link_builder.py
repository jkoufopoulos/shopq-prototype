...
def test_client_label_link_known_labels():
    ...
    assert (
        ...
        == "https://mail.google.com/mail/u/0/#label/ShopQ%2FAction-Required"
    )
    assert (
        GmailLinkBuilder.client_label_link("receipts")
        == "https://mail.google.com/mail/u/0/#label/ShopQ%2FReceipts"
    )
    assert (
        GmailLinkBuilder.client_label_link("messages")
        == "https://mail.google.com/mail/u/0/#label/ShopQ%2FMessages"
    )
    assert (
        GmailLinkBuilder.client_label_link("everything-else")
        == "https://mail.google.com/mail/u/0/#label/ShopQ%2FEverything-Else"
    )


def test_client_label_link_unknown_label():
    """Test fallback for unknown client label."""
    # Should fallback to generic ShopQ/{label} format
    result = GmailLinkBuilder.client_label_link("custom-label")
    assert "ShopQ%2Fcustom-label" in result
...
def test_client_label_link_url_encoding():
    ...
    # All our labels have ShopQ/{label} format, slash should become %2F
...
def test_build_client_label_links():
    ...
    for _label, url in links.items():
        ...
        assert "ShopQ%2F" in url  # Forward slash should be URL-encoded
...
def test_render_label_summary_prose_all_labels():
    ...
    assert 'href="https://mail.google.com/mail/u/0/#label/ShopQ%2FReceipts"' in result
...
def test_render_label_summary_prose_single_label():
    ...
    assert (
        result
        == 'The rest is <a href="https://mail.google.com/mail/u/0/#label/ShopQ%2FReceipts">5 receipts</a>.'
    )
