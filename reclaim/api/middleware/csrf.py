"""
CSRF Protection Middleware for Reclaim API.
...
# Set RECLAIM_EXTENSION_IDS as comma-separated list of allowed extension IDs
# In development, unpacked extensions have dynamic IDs, so we allow configuration
ALLOWED_EXTENSION_IDS = set(filter(None, os.getenv("RECLAIM_EXTENSION_IDS", os.getenv("SHOPQ_EXTENSION_IDS", "")).split(",")))
        ...
        if origin and origin.startswith("chrome-extension://"):
            ...
            if not ALLOWED_EXTENSION_IDS:
                ...
                raise HTTPException(
                    ...
                    detail="Extension not authorized (configure RECLAIM_EXTENSION_IDS)",
                )
            ...
            try:
                ...
                if referer_origin.startswith("chrome-extension://"):
                    ...
                    if (
                        ...
                        and os.getenv("RECLAIM_ENV", os.getenv("SHOPQ_ENV", "development")) == "development"
                    ):
                        return await call_next(request)
                    if ext_id in ALLOWED_EXTENSION_IDS:
                        return await call_next(request)
