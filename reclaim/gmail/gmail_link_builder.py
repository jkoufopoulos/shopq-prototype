...
class GmailLinkBuilder:
    ...
    CATEGORY_LABEL_MAP: dict[str, str] = {
        "Receipts": "label:Reclaim-Receipts",
        "Messages": "label:Reclaim-Messages",
        "Action Required": "label:Reclaim-Action-Required",
        "Everything Else": "label:Reclaim-Everything-Else",
    }
    ...
    def category_search_link(cls, category: str, days: int | None = None) -> str:
        ...
        if not label_query:
            ...
            label_query = f"label:Reclaim-{sanitized}"
    ...
    def unfeatured_items_link(cls, hours: int = 48) -> str:
        ...
        # Search for any Reclaim-labeled items from last N days (4-label system)
        query = (
            "("
            "label:Reclaim-Receipts OR label:Reclaim-Messages OR "
            "label:Reclaim-Action-Required OR label:Reclaim-Everything-Else"
            f") newer_than:{days}d"
        )
    ...
    def earlier_threads_link(cls, days: int = 7) -> str:
        ...
        Side Effects:
            None (pure function - builds URL string only)
        """
        # Search for Reclaim-labeled items older than N days (4-label system)
        query = (
            "("
            "label:Reclaim-Receipts OR label:Reclaim-Messages OR "
            "label:Reclaim-Action-Required OR label:Reclaim-Everything-Else"
    ...
    def action_required_link(cls, days: int = 7) -> str:
        ...
        Side Effects:
            None (pure function - builds URL string only)
        """
        query = f"label:Reclaim-Action-Required newer_than:{days}d"
        return cls.search_link(query)

    # Client label mappings (from TAXONOMY.md)
    CLIENT_LABEL_MAP: dict[str, str] = {
        "action-required": "Reclaim/Action-Required",
        "receipts": "Reclaim/Receipts",
        "messages": "Reclaim/Messages",
        "everything-else": "Reclaim/Everything-Else",
    }
    ...
    def client_label_link(cls, client_label: str) -> str:
        ...
        Side Effects:
            None (pure function - builds URL string only)
        """
        label_name = cls.CLIENT_LABEL_MAP.get(client_label, f"Reclaim/{client_label}")
    ...
    def render_type_counts_line(cls, type_counts: dict[str, int] | None) -> str:
        ...
        that navigate to the corresponding Reclaim type label in Gmail.
        ...
        # Map type to Gmail label name (Reclaim/Type format with capitalization)
        type_to_gmail_label = {
            "newsletter": "Reclaim/Newsletter",
            "notification": "Reclaim/Notification",
            "receipt": "Reclaim/Receipt",
            "promotion": "Reclaim/Promotion",
            "message": "Reclaim/Message",
            "event": "Reclaim/Event",
            "otp": "Reclaim/OTP",
            "shipping": "Reclaim/Shipping",
            "order": "Reclaim/Order",
            "other": "Reclaim/Other",
        }
