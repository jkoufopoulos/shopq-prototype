...
- Encryption key must be set via RECLAIM_ENCRYPTION_KEY environment variable
...
from reclaim.storage import BaseRepository
...
class UserCredentialsRepository(BaseRepository):
    ...
    def _get_cipher(self) -> Fernet:
        ...
        Raises:
            ValueError: If RECLAIM_ENCRYPTION_KEY is not set
        """
        encryption_key = os.getenv("RECLAIM_ENCRYPTION_KEY", os.getenv("SHOPQ_ENCRYPTION_KEY"))

        if not encryption_key:
            raise ValueError(
                "RECLAIM_ENCRYPTION_KEY environment variable must be set. "
    ...
    def store_credentials(
        ...
        Side Effects:
            - Inserts or updates row in user_credentials table in reclaim.db
    ...
    def delete_credentials(self, user_id: str) -> None:
        ...
        Side Effects:
            - Deletes row from user_credentials table in reclaim.db
