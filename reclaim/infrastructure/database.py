...
**DATABASE POLICY**: Reclaim uses ONE SQLite database: reclaim/data/reclaim.db
...
# Database path
DB_PATH = Path(__file__).parent.parent / "data" / "reclaim.db"
TEST_DB_PATH = Path(__file__).parent.parent / "data" / "reclaim_test.db"

# Legacy path for migration detection
_LEGACY_DB_PATH = Path(__file__).parent.parent / "data" / "shopq.db"
...
class DatabaseConnectionPool:
    ...
    def _create_connection(self):
        ...
        Side Effects:
            - Opens database connection to reclaim.db
    ...
    def _initialize_pool(self):
        ...
        Side Effects:
            - Creates database connections to reclaim.db
...
def get_pool() -> DatabaseConnectionPool:
    ...
    Side Effects:
        ...
        - Initializes database connection pool (opens connections to reclaim.db)
...
def get_db_path() -> Path:
    ...
    Checks RECLAIM_DB_PATH / SHOPQ_DB_PATH env var first,
    falls back to default location.
    """
    if env_path := os.getenv("RECLAIM_DB_PATH", os.getenv("SHOPQ_DB_PATH")):
...
def get_db_connection() -> Generator[sqlite3.Connection, None, None]:
    ...
    if not db_path.exists():
        # Check for legacy shopq.db that needs renaming
        if _LEGACY_DB_PATH.exists():
            raise FileNotFoundError(
                f"Database not found: {db_path}\n"
                f"Legacy database exists at {_LEGACY_DB_PATH}\n"
                f"Rename it: mv {_LEGACY_DB_PATH} {db_path}"
            )
        raise FileNotFoundError(
            f"Database not found: {db_path}\nRun: python -m reclaim.infrastructure.database_schema"
        )
...
def db_transaction() -> Generator[sqlite3.Connection, None, None]:
    ...
    Side Effects:
        - Commits transaction to reclaim.db on success (writes changes to disk)
...
def execute_query(
    ...
    Side Effects:
        - Executes SQL query on reclaim.db
...
def validate_schema() -> bool:
    ...
    """
    from reclaim.infrastructure.database_schema import validate_schema as _validate_schema
...
def checkpoint_wal() -> dict[str, Any]:
    ...
    Side Effects:
    - Writes WAL frames to main database file (reclaim.db)
    - Truncates WAL file (reclaim.db-wal) to zero bytes
...
def get_test_db_connection() -> sqlite3.Connection:
    ...
    Side Effects:
        - Opens connection to reclaim_test.db (creates file if doesn't exist)
...
def init_database() -> None:
    ...
    """
    from reclaim.infrastructure.database_schema import init_database as _init_database

    # Warn if legacy shopq.db exists but reclaim.db doesn't
    db_path = get_db_path()
    if not db_path.exists() and _LEGACY_DB_PATH.exists():
        logger.warning(
            "Legacy database found at %s but %s does not exist. "
            "Rename it: mv %s %s",
            _LEGACY_DB_PATH,
            db_path,
            _LEGACY_DB_PATH,
...
# See @app.on_event("startup") in reclaim/api/app.py
