...
from mailq.observability.confidence import (
...
class ConfidenceLogger:
    ...
    Uses the centralized database connection pool (mailq.db).
    ...
    def _ensure_schema(self) -> None:
        ...
        Side Effects:
            - Creates confidence_logs table in mailq.db (if not exists)
    ...
    def log_classification(
        ...
        Side Effects:
            - Inserts row into confidence_logs table in mailq.db
            - Commits transaction to database
        """
        import json

        from mailq.utils.versioning import get_version_metadata
    ...
    def clear_old_logs(self, days: int = 90) -> int:
        ...
        Side Effects:
            - Deletes rows from confidence_logs table in mailq.db
            - Commits transaction to database
