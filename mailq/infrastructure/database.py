...
TEST_DB_PATH = Path(__file__).parent.parent / "data" / "mailq_test.db"
...
class DatabaseConnectionPool:
    ...
    def _create_connection(self):
        ...
        Side Effects:
            - Opens database connection to mailq.db
    ...
    def _initialize_pool(self):
        ...
        Side Effects:
            - Creates database connections to mailq.db
...
def get_pool() -> DatabaseConnectionPool:
    ...
    Side Effects:
        ...
        - Initializes database connection pool (opens connections to mailq.db)
...
def get_db_path() -> Path:
    ...
    """
    if env_path := os.getenv("MAILQ_DB_PATH"):
...
def db_transaction() -> Generator[sqlite3.Connection, None, None]:
    ...
    Side Effects:
        - Commits transaction to mailq.db on success (writes changes to disk)
...
def execute_query(
    ...
    Side Effects:
        - Executes SQL query on mailq.db
...
def validate_schema() -> bool:
    ...
    """
    from mailq.infrastructure.database_schema import validate_schema as _validate_schema
...
def checkpoint_wal() -> dict[str, Any]:
    ...
    Side Effects:
    - Writes WAL frames to main database file (mailq.db)
    - Truncates WAL file (mailq.db-wal) to zero bytes
...
def get_test_db_connection() -> sqlite3.Connection:
    ...
    Side Effects:
        - Opens connection to mailq_test.db (creates file if doesn't exist)
...
def init_database() -> None:
    ...
    Side Effects:
    - Creates tables in mailq.db if they don't exist
    - Creates indexes for query performance
    - Creates mailq/data/ directory if needed
    - Idempotent: safe to run multiple times
    """
    from mailq.infrastructure.database_schema import init_database as _init_database
...
# See @app.on_event("startup") in mailq/api.py
