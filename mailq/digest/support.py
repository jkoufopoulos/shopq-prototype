...
def normalize_email_payload(payload: dict[str, Any]) -> NormalizedEmail:
    ...
    if os.getenv("MAILQ_DEBUG_CLASSIFICATION", "").lower() in ("true", "1", "yes"):
...
class ValidatedDigestEntry(BaseModel):
    ...
    Note: This is distinct from mailq.contracts.entities.DigestEntity (Protocol)
...
def adapt_entity_for_digest(entity: Any) -> DigestEntity:
    ...
    from mailq.contracts.entities import DigestEntity as DigestEntityProtocol
