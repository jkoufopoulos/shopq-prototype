...
All confidence-related thresholds for the MailQ classification system.
Changes here propagate across backend and (via API) to frontend.

IMPORTANT: All thresholds are loaded from config/mailq_policy.yaml.
...
def _load_policy_config() -> dict[str, Any]:
    """
    Load configuration from mailq_policy.yaml.
    ...
    possible_paths = [
        Path(__file__).parent.parent.parent / "config" / "mailq_policy.yaml",
        Path(__file__).parent.parent / "config" / "mailq_policy.yaml",
        Path("config/mailq_policy.yaml"),
    ]
    ...
    logger.warning("mailq_policy.yaml not found, using hardcoded defaults")
    return {}
