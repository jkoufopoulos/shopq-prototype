"""Confidence monitoring endpoints for MailQ API.
...
async def get_confidence_thresholds() -> dict[str, Any]:
    ...
    Returns all confidence-related thresholds from mailq/config/confidence.py
    so frontend can use consistent values instead of hard-coding them.

    Side Effects:
        None (pure function - reads config constants only)
    """
    from mailq.observability.confidence import get_all_thresholds
...
async def get_confidence_stats(days: int = 7) -> dict[str, Any]:
    ...
    Side Effects:
        - Reads from confidence_logs table in mailq.db
    """
