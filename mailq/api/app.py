"""FastAPI server for MailQ email classification"""
...
from mailq.api.middleware.rate_limit import RateLimitMiddleware  # noqa: E402
from mailq.api.middleware.security_headers import SecurityHeadersMiddleware  # noqa: E402
from mailq.api.models import (
    EmailBatch,
    OrganizeResponse,
    SummaryRequest,
    VerifyRequest,
...
from mailq.api.routes.categories import router as categories_router
from mailq.api.routes.categories import set_category_manager, set_classifier
from mailq.api.routes.confidence import router as confidence_router
from mailq.api.routes.debug import router as debug_router
from mailq.api.routes.debug import set_last_batch
from mailq.api.routes.debug_ab_testing import router as debug_ab_testing_router
from mailq.api.routes.feature_gates import router as feature_gates_router  # noqa: E402
from mailq.api.routes.feedback import router as feedback_router  # noqa: E402
from mailq.api.routes.feedback import set_feedback_manager  # noqa: E402
from mailq.api.routes.health import router as health_router
from mailq.api.routes.linker import router as linker_router  # noqa: E402
from mailq.api.routes.rules import router as rules_router
from mailq.api.routes.test import router as test_router  # noqa: E402
from mailq.api.routes.tracking import router as tracking_router  # noqa: E402
from mailq.classification.memory_classifier import MemoryClassifier  # noqa: E402
from mailq.concepts.feedback import FeedbackManager  # noqa: E402
from mailq.digest.category_manager import CategoryManager  # noqa: E402
from mailq.digest.context_digest import ContextDigest  # noqa: E402
from mailq.digest.support import normalize_email_payload  # noqa: E402
from mailq.infrastructure.database import init_database  # noqa: E402
from mailq.observability.logging import get_logger  # noqa: E402
from mailq.observability.telemetry import log_event
from mailq.observability.tracking import EmailThreadTracker  # noqa: E402
from mailq.runtime.flags import is_enabled
from mailq.utils.redaction import redact  # noqa: E402

# Load environment variables from .env file
load_dotenv()

app = FastAPI(title="MailQ API", version="2.0.0-mvp")
...
async def validation_exception_handler(
    ...
    """
    from mailq.observability.telemetry import counter
...
MAILQ_EXTENSION_ID = os.getenv("MAILQ_EXTENSION_ID", "")
...
env = os.getenv("MAILQ_ENV", "development")
if env == "production":
    if not os.getenv("MAILQ_ADMIN_API_KEY"):
        logger.critical(
            "=" * 70 + "\n"
            "🔴 CRITICAL SECURITY MISCONFIGURATION\n"
            "🔴 MAILQ_ADMIN_API_KEY is not set in production!\n"
    ...
    if not os.getenv("MAILQ_ADMIN_API_KEY"):
        logger.warning(
            "=" * 70 + "\n"
            "⚠️  SECURITY: MAILQ_ADMIN_API_KEY not set\n"
...
def _wal_checkpoint_loop():
    ...
    Side Effects:
        - Calls checkpoint_wal() which writes to mailq.db (WAL checkpoint operation)
...
async def organize_emails(batch: EmailBatch) -> dict[str, Any]:
    ...
    Side Effects:
        - Calls classify_batch() which may write to confidence_logs table in mailq.db
    ...
    """
    from mailq.api.routes.organize import (
        classify_batch,
...
async def verify_classification(request: VerifyRequest) -> dict[str, Any]:
    ...
    Side Effects:
        ...
        - Logs telemetry events via log_event()
    """
    from mailq.api.routes.verify import verify_classification as run_verify
...
async def generate_context_digest_v1(request: SummaryRequest) -> dict[str, Any]:
    ...
    Side Effects:
        - Calls mailq.context_digest.generate() which may call LLM APIs
    ...
    if os.getenv("MAILQ_DEBUG_CLASSIFICATION", "").lower() in ("true", "1", "yes") and emails:
