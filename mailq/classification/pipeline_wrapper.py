...
This module bridges the existing mailq/api.py with the refactored backend.
...
from mailq.gmail.api_bridge import api_email_to_parsed, classified_to_api_result
...
class RefactoredPipelineClassifier:
    ...
    def __init__(self):
        ...
        self.use_llm = os.getenv("MAILQ_USE_LLM", "true").lower() == "true"
    ...
    def _create_fallback_result(self, _from_field: str) -> dict[str, Any]:
        ...
        return {
            ...
            "labels_conf": {"MailQ-Everything-Else": 0.5},
            "gmail_labels": ["MailQ-Everything-Else"],
            "gmail_labels_conf": {"MailQ-Everything-Else": 0.5},
...
def classify_batch_refactored(
    ...
    """
    if use_llm is None:
        use_llm = os.getenv("MAILQ_USE_LLM", "true").lower() == "true"
