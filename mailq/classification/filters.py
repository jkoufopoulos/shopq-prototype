...
- Self-email detection (MailQ digest exclusion)
...
def is_self_email(email: dict, user_email: str | None = None) -> bool:
    """
    Check if email is MailQ's own digest email.
    ...
    Returns:
        True if email should be EXCLUDED (MailQ digest email)
    ...
    # Pattern 2: Has MailQ/Digest label
    if "MailQ/Digest" in labels or "MAILQ/DIGEST" in [label.upper() for label in labels]:
        return True
...
def filter_self_emails(emails: list[dict], user_email: str | None = None) -> list[dict]:
    """
    Filter out MailQ's own digest emails.
...
def is_github_quality_issue(email: dict) -> bool:
    ...
    Filters out GitHub notifications about MailQ quality issues to prevent
