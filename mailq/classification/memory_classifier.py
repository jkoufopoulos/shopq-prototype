...
from mailq.classification.mapper import map_to_gmail_labels, validate_classification_result
...
from mailq.observability.confidence import LEARNING_MIN_CONFIDENCE
...
class MemoryClassifier:
    ...
    def _rule_to_semantic(self, rule_result: dict[str, Any], from_field: str) -> dict[str, Any]:
        ...
        category = rule_result["category"]  # e.g., "Promotions" or "MailQ-Promotions"
        conf = rule_result["confidence"]

        # Strip MailQ- prefix if present (handle both MailQ- and MailQ/ formats)
        category_clean = category.replace("MailQ-", "").replace("MailQ/", "")
