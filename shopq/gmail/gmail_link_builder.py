...
class GmailLinkBuilder:
    ...
    CATEGORY_LABEL_MAP: dict[str, str] = {
        "Receipts": "label:ShopQ-Receipts",
        "Messages": "label:ShopQ-Messages",
        "Action Required": "label:ShopQ-Action-Required",
        "Everything Else": "label:ShopQ-Everything-Else",
    }
    ...
    def category_search_link(cls, category: str, days: int | None = None) -> str:
        ...
        if not label_query:
            ...
            label_query = f"label:ShopQ-{sanitized}"
    ...
    def unfeatured_items_link(cls, hours: int = 48) -> str:
        ...
        # Search for any ShopQ-labeled items from last N days (4-label system)
        query = (
            "("
            "label:ShopQ-Receipts OR label:ShopQ-Messages OR "
            "label:ShopQ-Action-Required OR label:ShopQ-Everything-Else"
            f") newer_than:{days}d"
        )
    ...
    def earlier_threads_link(cls, days: int = 7) -> str:
        ...
        Side Effects:
            None (pure function - builds URL string only)
        """
        # Search for ShopQ-labeled items older than N days (4-label system)
        query = (
            "("
            "label:ShopQ-Receipts OR label:ShopQ-Messages OR "
            "label:ShopQ-Action-Required OR label:ShopQ-Everything-Else"
    ...
    def action_required_link(cls, days: int = 7) -> str:
        ...
        Side Effects:
            None (pure function - builds URL string only)
        """
        query = f"label:ShopQ-Action-Required newer_than:{days}d"
        return cls.search_link(query)
    ...
    def client_label_link(cls, client_label: str) -> str:
        ...
        Side Effects:
            None (pure function - builds URL string only)
        """
        label_name = cls.CLIENT_LABEL_MAP.get(client_label, f"ShopQ/{client_label}")
    ...
    def render_type_counts_line(cls, type_counts: dict[str, int] | None) -> str:
        ...
        that navigate to the corresponding ShopQ type label in Gmail.
        ...
        # Map type to Gmail label name (ShopQ/Type format with capitalization)
        type_to_gmail_label = {
            "newsletter": "ShopQ/Newsletter",
            "notification": "ShopQ/Notification",
            "receipt": "ShopQ/Receipt",
            "promotion": "ShopQ/Promotion",
            "message": "ShopQ/Message",
            "event": "ShopQ/Event",
            "otp": "ShopQ/OTP",
            "shipping": "ShopQ/Shipping",
            "order": "ShopQ/Order",
            "other": "ShopQ/Other",
        }
