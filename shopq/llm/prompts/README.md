...
```python
from shopq.prompts import get_classifier_prompt, get_verifier_prompt
...
```python
from shopq.prompts import reload_prompts
...
```bash
# View prompt changes
git diff shopq/prompts/classifier_prompt.txt

# Commit prompt improvements
git add shopq/prompts/
...
**Fix**: Check the file exists in `shopq/prompts/` directory.
