...
from shopq.gmail.gmail_link_builder import GmailLinkBuilder
...
class CardRenderer:
    ...
    def _render_email_summary(self, label_counts: dict[str, int] | None) -> str:
        ...
        html_parts.append('<div class="footer-brand">ShopQ · Settings</div>')
    ...
    def _render_noise_summary(self, noise_breakdown: dict[str, int], time_hours: int = 24) -> str:
        ...
        # All noise categories map to ShopQ-Everything-Else
        category_to_label = {
            "newsletters": "ShopQ-Everything-Else",
            "promotional": "ShopQ-Everything-Else",
            "receipts": "ShopQ-Receipts",
            "messages": "ShopQ-Messages",
            "events": "ShopQ-Everything-Else",
            "old_calendar": "ShopQ-Everything-Else",
            "subscription_renewal": "ShopQ-Everything-Else",
            "social_notifications": "ShopQ-Everything-Else",
            "read_receipts": "ShopQ-Everything-Else",
            "other": "ShopQ-Everything-Else",
        }

        # Group counts by Gmail label (multiple categories can map to same label)
        label_counts = {}
        label_friendly_names = {
            "ShopQ-Everything-Else": "everything else",
            "ShopQ-Receipts": "receipts",
            "ShopQ-Messages": "messages",
            "ShopQ-Action-Required": "action items",
        }

        for category, count in noise_breakdown.items():
            label = category_to_label.get(category, "ShopQ-Everything-Else")

            # Skip compound queries for now (they're for "other" category)
            if " OR " in label:
                # For "other", add to Everything-Else
                label = "ShopQ-Everything-Else"
        ...
        for label, count in sorted(label_counts.items(), key=lambda x: -x[1]):
            ...
            friendly_name = label_friendly_names.get(label, label.replace("ShopQ-", "").lower())
            ...
            # archived emails (ShopQ archives after organizing).
