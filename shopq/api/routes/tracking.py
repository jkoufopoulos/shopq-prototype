...
async def list_sessions() -> dict[str, Any]:
    ...
    Side Effects:
        - Reads from email_threads table in shopq.db
    """
...
async def get_session_report(session_id: str) -> dict[str, Any]:
    ...
    Side Effects:
        - Reads from email_threads and summary_emails tables in shopq.db
    """
...
async def export_session_csv(session_id: str) -> dict[str, Any]:
    ...
    fd, path = tempfile.mkstemp(suffix=".csv", prefix=f"shopq_session_{session_id}_")
    os.close(fd)
    ...
    return {
        "session_id": session_id,
        "csv": csv_content,
        "filename": f"shopq_session_{session_id}.csv",
    }
