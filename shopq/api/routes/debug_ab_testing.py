...
async def get_ab_testing_summary(limit: int = Query(default=100)) -> dict[str, Any]:
    ...
    Side Effects:
        - Reads from ab_test_runs table in shopq.db via runner.get_summary_stats()
    """
...
async def get_recent_ab_tests(limit: int = Query(default=20)) -> dict[str, Any]:
    ...
    Side Effects:
        - Reads from ab_test_runs table in shopq.db
    """
...
async def get_ab_test_details(test_id: str) -> dict[str, Any]:
    ...
    Side Effects:
        - Reads from ab_test_runs and ab_test_metrics tables in shopq.db
    """
    from shopq.infrastructure.database import get_db_connection
