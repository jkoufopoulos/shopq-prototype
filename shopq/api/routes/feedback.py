...
from shopq.api.routes.dashboard import render_dashboard
from shopq.concepts.feedback import FeedbackManager
...
def _teach_rules_engine(
    ...
    Side Effects:
        - Writes to rules table in shopq.db (via RulesEngine.learn_from_classification)
        - Logs learning events
    """
    from datetime import datetime

    from shopq.classification.classifier import get_classifier
...
async def record_feedback(feedback: FeedbackInput) -> dict[str, Any]:
    ...
    Side Effects:
        - Writes to corrections table in shopq.db (via feedback_manager.record_correction)
        - Writes to learned_patterns table in shopq.db (FeedbackManager learning)
        - Writes to rules table in shopq.db (RulesEngine learning via EmailClassifier)
...
async def get_feedback_stats(user_id: str = "default") -> dict[str, Any]:
    ...
    Side Effects:
        - Reads from corrections and learned_patterns tables in shopq.db
    """
    if not feedback_manager:
        raise HTTPException(status_code=500, detail="Feedback manager not initialized")
...
async def get_pending_rules(user_id: str = "default") -> dict[str, Any]:
    ...
    Side Effects:
        - Reads from learned_patterns table in shopq.db
    """
    from shopq.classification.rules_manager import RulesManager
