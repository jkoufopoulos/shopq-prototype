"""Rules management endpoints for ShopQ API.
...
from shopq.api.models import RuleCreate, RuleUpdate
from shopq.api.utils import get_client_ip
from shopq.classification.rules_manager import (
    add_rule,
    delete_rule,
    get_rule_stats,
    get_rules,
...
async def create_rule(
    ...
    Side Effects:
        - Writes to rules table in shopq.db via add_rule()
...
async def modify_rule(
    ...
    Side Effects:
        - Updates rules table in shopq.db via update_rule()
        - Logs audit event via log_event()
    """
    try:
...
async def remove_rule(
    ...
    Side Effects:
        - Deletes from rules table in shopq.db via delete_rule()
        - Logs audit event via log_event()
    """
    try:
