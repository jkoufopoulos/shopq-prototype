...
- Added `importance_conf` field to `ClassificationContract` in `shopq/storage/classification.py`
- Fixed `TypeMapper` path resolution (was looking in `shopq/config/` instead of `config/`)
...
- `shopq/digest/context_enricher.py`
- `shopq/digest/formatting.py`
- `shopq/digest/hybrid_digest_renderer.py`
- `shopq/digest/llm_section_classifier.py`
- `shopq/classification/importance_classifier.py`
- `shopq/classification/importance/heuristics.py`
- `shopq/llm/prompts/section_assignment_prompt.txt`
...
- Updated `mapper.js` to use new `ShopQ-*` label format
...
git log --follow -p -- shopq/digest/context_digest.py
```
