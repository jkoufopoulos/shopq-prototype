#!/usr/bin/env python3
"""
Apply Type Corrections to GDS

Reads decisions from type_review_decisions.csv and applies them to gds-2.0.csv.
Creates a backup before making changes.

Usage:
    python3 scripts/evals/tools/apply_type_corrections.py [--dry-run]

Side Effects:
- Creates backup of gds-2.0.csv
- Modifies gds-2.0.csv email_type values based on review decisions
"""
...
def apply_corrections(
    ...
    for row in rows:
        ...
        if email_id in decisions:
            ...
            old_type = row["email_type"]
            new_type = decision["correct_value"]
...
def main():
    ...
    print("=" * 60)
    print("Apply Type Corrections to GDS")
    print("=" * 60)
    ...
    print("-" * 60)
    print(f"Total: {len(changes)} email_type values {'would be ' if dry_run else ''}updated")
