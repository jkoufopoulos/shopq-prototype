...
echo "📊 Step 1: Checking for new digest sessions..."
echo ""
...
echo "🤖 Step 2: Running Claude analysis on digest sessions..."
echo ""
...
echo "📈 Step 3: Checking for digest quality logs..."
echo ""

if [ -d "$PROJECT_ROOT/quality_logs" ] && [ "$(ls -A "$PROJECT_ROOT/quality_logs"/input_emails_*.json 2>/dev/null)" ]; then
  echo "✅ Found digest quality logs"
  echo ""

  echo "📊 Generating digest comparison (ACTUAL vs IDEAL)..."
  echo ""
  ...
  if [ $? -eq 0 ]; then
    ...
    if [ -n "$LATEST_COMPARISON" ]; then
      echo ""
      echo "✅ Digest comparison generated: $LATEST_COMPARISON"
      echo ""
      echo "📝 Review the comparison to identify gaps:"
      echo "   open $LATEST_COMPARISON"
      echo ""
    fi
  else
    echo "⚠️  Digest comparison generation failed (non-critical)"
  fi
else
  echo "ℹ️  No digest quality logs found - skipping comparison"
...
echo "🐙 Step 4: Creating GitHub issues for quality problems..."
echo ""
...
EOF
    ...
    echo "✅ No new issues to create (all issues already on GitHub)"
  fi
else
  echo "⚠️  Skipping GitHub issue creation (GITHUB_TOKEN not set)"
fi
